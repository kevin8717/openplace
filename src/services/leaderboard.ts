...
export class LeaderboardService {
	...
	private async updateAllianceLeaderboard(mode: LeaderboardMode, dateFilter: any): Promise<void> {
		...
		// For time-based modes, use a single aggregated query instead of N+1 loop
		const paintedAtGte = dateFilter?.paintedAt?.gte;
		const sqlFragments: any[] = [
			Prisma.sql`SELECT u.allianceId, COUNT(*) AS \`count\``,
			Prisma.sql`FROM Pixel p`,
			Prisma.sql`INNER JOIN User u ON p.paintedBy = u.id`,
			Prisma.sql`WHERE u.allianceId IS NOT NULL`,
			Prisma.sql`AND u.allianceJoinedAt IS NOT NULL`,
			Prisma.sql`AND u.allianceJoinedAt <= p.paintedAt`,
		];
		if (paintedAtGte) {
			sqlFragments.push(Prisma.sql`AND p.paintedAt >= ${paintedAtGte}`);
		}
		sqlFragments.push(Prisma.sql`GROUP BY u.allianceId`);
		sqlFragments.push(Prisma.sql`ORDER BY \`count\` DESC`);
		sqlFragments.push(Prisma.sql`LIMIT 50`);

		const pixelCounts = await prisma.$queryRaw<{ allianceId: number; count: bigint }[]>(
			Prisma.join(sqlFragments, " ")
		);

		const entries = pixelCounts
			.filter(row => row.allianceId != null && Number(row.count) > 0)
			.map((row, index) => ({
				type: "alliance" as const,
				mode,
				entityId: row.allianceId,
				rank: index + 1,
				pixelsPainted: Number(row.count)
			}));
